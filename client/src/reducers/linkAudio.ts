...
export interface LinkAudioState {
	...
	// display order, as slot keys — server-authoritative, so we render through these rather
	// than sorting the maps locally
	sourceOrder: string[];
	sinkOrder: string[];
...
export const linkAudio = (state: LinkAudioState = defaultState, action: LinkAudioAction): LinkAudioState => {
	...
	switch (action.type) {
		...
		case LinkAudioActionType.INIT: {
			const { available, peers, peerName, latencyMs, syncToIncoming, sourceOrder, sinkOrder, sources, sinks } = action.payload;
			...
			return { ...state, available, peers, peerName, latencyMs, syncToIncoming, sourceOrder, sinkOrder, sources, sinks };
		}
		...
		case LinkAudioActionType.SET_SOURCE_ORDER: {
			return { ...state, sourceOrder: action.payload.order };
		}

		case LinkAudioActionType.SET_SINK_ORDER: {
			return { ...state, sinkOrder: action.payload.order };
		}
		...
		case LinkAudioActionType.UPDATE_SOURCE: {
			...
			const existing = state.sources.get(key) || new LinkAudioSourceRecord({ key });
		...
		case LinkAudioActionType.UPDATE_SINK: {
			const { key, changes } = action.payload;
			const existing = state.sinks.get(key) || new LinkAudioSinkRecord({ key });
